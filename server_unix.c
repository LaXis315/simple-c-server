...
//#include <inetdb.h> da utilizzare per ottenere facilmente interfacce del computer e creare sockaddr
...
int socket_and_binding(char* port){ //addr_in verà popolato di dati e poi castato in sockaddr per essere letto da binding
	
	//USEREMO int bind(int sockfd, const struct sockaddr *addr, socklen_t addrlen);
    ...
    */
    ...
    hints.ai_addr = NULL;
   	...
   	int value = getaddrinfo(NULL, port, &hints, &result);
...
int main(){
	...
	int sockfd = socket_and_binding("8691");  //bindiamo il socket alla nostra porta e indirizzo
